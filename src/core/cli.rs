...
pub fn cmd_token_list(format: OutputFormat, passphrase: &str) -> Result<()> {
    ...
    match format {
        OutputFormat::Human => {
            ...
            println!("{:<30} {:<20} {}", "NAME", "EXPIRES", "STATUS");
            println!("{}", "-".repeat(60));
            
            for secret in secrets {
                let status = if secret.is_expired() { "⚠️ EXPIRED" } else { "✓" };
                println!(
                    "{:<30} {:<20} {}",
                    secret.name,
...
"#;
        ...
        assert_eq!(secrets.get("DATABASE_URL"), Some(&"postgres://localhost/db".to_string()));
        ...
        assert_eq!(secrets.get("SPACES"), Some(&"value with spaces".to_string()));
    }
