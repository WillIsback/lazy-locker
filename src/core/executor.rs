...
pub fn generate_env_file(store: &SecretsStore, key: &[u8], output_path: &std::path::PathBuf) -> Result<()> {
    let secrets = store.decrypt_all(key)?;
...
pub fn export_to_json(store: &SecretsStore, key: &[u8], output_path: &std::path::PathBuf) -> Result<()> {
    let secrets = store.decrypt_all(key)?;
